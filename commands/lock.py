...
@has_role_permission('lock')
@commands.has_permissions(manage_channels=True)
async def lock(ctx, *, channel_arg=None):
    guild = ctx.guild
    ...
    try:
        await channel.set_permissions(everyone, overwrite=overwrite, reason=f"Locked by {ctx.author}")
        ...
        await ctx.send(f"Failed to lock the channel. Error: **{e}**", delete_after=3)
